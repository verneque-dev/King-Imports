...
export default async function Home({ searchParams }: Props) {
  ...
  const res = await fetch(`${urlApi}/api/produtos/?search=${query}&page=1&take=5`, {
    cache: "no-store"
  })
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
        ...
        {produtos.map((produto) => {
          return (
            <Link href={`./produto/${produto.id_produtos}`} key={produto.id_produtos}>
              <div className="flex flex-col rounded-lg overflow-hidden shadow-lg hover:scale-105 transition-transform">
